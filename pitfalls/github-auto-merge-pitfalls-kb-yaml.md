...
metadata:
  ...
  root_cause: auto-merge workflow 检查只允许新增 pitfalls/ 下的文件（diff-filter 排除 D/M）；schema 变更属于控制层低频操作，不应自动合入
...
auto-merge workflow 检查只允许新增 pitfalls/ 下的文件（diff-filter 排除 D/M）；schema 变更属于控制层低频操作，不应自动合入
